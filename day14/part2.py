...
def compute(s: str) -> int:
    ...
    FLOOR = max(rock_points, key = lambda x: x[1])[1] + 2
    sand_points = set()

    done = False
    curr_position = ENTRY_POINT
    # print(f'floor depth is {FLOOR}')
    ...
    while not done:
        ...
        # check if entry point blocked
        if ENTRY_POINT in sand_points:
            done = True
        ...
        elif straight_down not in rock_points and straight_down not in sand_points:
