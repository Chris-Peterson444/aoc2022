...
def compute(s: str) -> int:
    ...
    for child in target_children:
        child_cost.append((child,child.compute_size()))
    child_cost.sort(key=lambda child: child[1])

    TOTAL = 70000000
    NEED = 30000000
    USED = tree_root.compute_size()
    for child, cost in child_cost:
        if TOTAL - USED + cost >= NEED:
            return cost
