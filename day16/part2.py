...
def compute(s: str) -> int:
    ...
    # Looks like saving the values helps in this case, so going with my original option 1
    ...
    answers = visit('AA', 26, [], 0, dict())

    # Can't overlap openinings with elephant
    potentials = [
        p1+p2 for visited_1, p1 in answers.items() for visited_2, p2 in answers.items()
            if visited_1.isdisjoint(visited_2)
    ]
